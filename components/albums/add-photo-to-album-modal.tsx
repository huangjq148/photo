...
type AddPhotoToAlbumModalProps = {
  open: boolean;
...
export function AddPhotoToAlbumModal({
  open,
  currentAlbumId,
  photoId,
  photoName,
  ...
  useEffect(() => {
    ...
    async function loadAlbums() {
      try {
        ...
        const items = (json.data ?? []).filter((album: AlbumItem) => album.id !== currentAlbumId);
        if (active) {
          setAlbums(items);
        }
      } catch (err) {
  ...
  async function addToAlbum(album: AlbumItem) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(json.error ?? "添加到相册失败");
      }

      message.success(`已添加到「${album.name}」`);
      onAdded?.();
      onClose();
    } catch (err) {
      const text = err instanceof Error ? err.message : "添加到相册失败";
      setError(text);
      message.error(text);
    } finally {
